...
@section('contents')
  ...
  <style>
    ...
    .nice-select { width: 100%; margin-bottom: 15px; border: 1px solid #bebebe;}

  </style>

  <section>
    <section class="p-5 d-100" id="delivery_agent_form">
      <div class="merchant-form-body container">
        <form action="#" method="POST">
          @csrf

          <div class="merchant-form__heading">Become a Agent</div>

          <div class="group-input-radio">
            <label for="delivery_agent">
              <input type="radio" class="delivery_agent radio form-input" checked name="delivery_agent" id="delivery_agent"> <span>Delivery Agent</span>
            </label>
            <label for="delivery_man">
              <input type="radio" class="delivery_man radio form-input" name="delivery_man" id="delivery_man"> <span>Delivery Man</span>
            </label>
          </div>
...
@push('scripts')
  <script>
    $(document).ready(function(){
      ...
      $(document).on('change click', '.delivery_agent', function() {
          $('.delivery_agent').prop('checked', true)
          $('.delivery_man').prop('checked', false)
          $('#delivery_agent_form').removeClass('d-none');
          $('#delivery_man_form').addClass('d-none');
      });

      $(document).on('change click', '.delivery_man', function() {
          $('.delivery_agent').prop('checked', false)
          $('.delivery_man').prop('checked', true)
          $('#delivery_agent_form').addClass('d-none');
          $('#delivery_man_form').removeClass('d-none');
      });
    });  
  </script>    
@endpush
...
<div class="group-input">
  <input type="text" placeholder="Merchant Pickup Area" class="form-control">
      ...
      // $('.delivery_agent').on('change click' ,function(){
        //   $('#delivery_agent_form').removeClass('d-none')
        //   $('#delivery_man_form').addClass('d-none')
        // });
  
        // $('.delivery_man').on('change click' ,function(){
        //   $('#delivery_agent_form').addClass('d-none')
        //   $('#delivery_man_form').removeClass('d-none')
        // }); --}}